/**
 * Bech32m decoder for Arkade addresses, returning the BTC pkScript that the
 * Arkade indexer expects in its `scripts=` query parameter.
 *
 ...
 *
 * Reference: BIP-350. Arkade addresses encode 65 bytes:
...
export function bech32mDecode(addr) {
  ...
  if (lower.length > 1023) throw new Error('Arkade address too long');
  const sep = lower.lastIndexOf('1');
  if (sep < 1 || sep + 7 > lower.length) {
    throw new Error('Arkade address: invalid bech32m structure');
  }
  const hrp = lower.substring(0, sep);
  const data = [];
  for (let i = sep + 1; i < lower.length; i++) {
    const idx = BECH32M_CHARSET.indexOf(lower.charAt(i));
    if (idx === -1) throw new Error('Arkade address: invalid bech32m character');
    data.push(idx);
  }
  if (bech32mPolymod([...bech32mHrpExpand(hrp), ...data]) !== BECH32M_CONST) {
    throw new Error('Arkade address: bech32m checksum mismatch');
  }
...
export function bech32mFromWords(words) {
  ...
  for (const v of words) {
    if (v < 0 || v > 31) throw new Error('Arkade address: invalid 5-bit value');
    acc = (acc << 5) | v;
    bits += 5;
    while (bits >= 8) {
      bits -= 8;
  ...
  if (bits >= 5 || (acc << (8 - bits)) & 0xff) {
    throw new Error('Arkade address: invalid bech32m padding');
  }
  return out;
}

/**
 * Decode an Arkade address into the BTC pkScript that the Arkade indexer expects in
...
export function arkAddressToPkScript(address) {
  ...
  if (data.length !== 65) {
    throw new Error(`Arkade address: expected 65-byte payload (1+32+32), got ${data.length}`);
  }
